        ...
        <header className="bg-white dark:bg-slate-800/50 dark:backdrop-blur-sm rounded-2xl shadow-2xl p-6 mb-6">
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
            <div>
              ...
              <p className="text-slate-500 dark:text-blue-100 text-base lg:text-lg">ISO 50001 System - Intelligent Energy Monitoring</p>
            </div>
            <div className="text-center sm:text-right">
              ...
              <p className="text-sm text-slate-500 dark:text-blue-200">Real-Time Update</p>
        ...
        <div className="bg-white dark:bg-slate-800 rounded-xl shadow-lg p-2 mb-6 flex gap-1 sm:gap-2 overflow-x-auto">
          {['overview', 'ai-workloads', 'iso-50001', 'enpi', 'comparison'].map(tab => (
            <button key={tab} onClick={() => setActiveTab(tab)} className={`flex-1 min-w-[120px] py-2.5 px-3 sm:py-3 sm:px-4 rounded-lg font-medium transition-all whitespace-nowrap text-sm sm:text-base ${activeTab === tab ? 'bg-blue-600 text-white shadow-md' : 'bg-transparent text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-slate-700'}`}>
              ...
              {tab === 'ai-workloads' && 'AI Workloads'}
              {tab === 'iso-50001' && 'ISO 50001'}
              {tab === 'enpi' && 'EnPI & Targets'}
          ...
          {activeTab === 'overview' && (
            <div className="space-y-6">
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                <StatCard icon={Activity} title="PUE (ISO 50001)" value={realTimeData.pue.toFixed(2)} unit="" trend={-currentISO.improvement / 10} color="#F59E0B" subtitle={`Target: ${dcType === 'underwater' ? '1.15' : '1.40'}`} />
              </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <div className="bg-white dark:bg-slate-800 rounded-xl shadow-lg p-6">
                  ...
                  <div className="space-y-4">
                    ...
                    {dcType === 'terrestrial' && (<div><div className="flex items-center justify-between mb-1"><span className="text-sm text-gray-600 dark:text-gray-400">Humidity</span><span className="font-bold text-lg text-blue-600">{realTimeData.humidity}%</span></div><div className="w-full bg-gray-200 dark:bg-slate-700 rounded-full h-2"><div className="bg-blue-600 h-2 rounded-full transition-all" style={{ width: `${realTimeData.humidity}%` }}></div></div><p className="text-xs text-gray-500 dark:text-gray-400 mt-1">ISO Range: 40-60%</p></div>)}
                    {dcType === 'underwater' && (<div className="bg-blue-50 dark:bg-blue-900/50 p-3 rounded-lg text-sm"><div className="flex items-center justify-between"><span className="text-blue-700 dark:text-blue-300 font-medium">Depth</span><span className="font-bold text-blue-900 dark:text-blue-200">117m</span></div><div className="flex items-center justify-between"><span className="text-blue-700 dark:text-blue-300 font-medium">Pressure</span><span className="font-bold text-blue-900 dark:text-blue-200">12.5 bar</span></div></div>)}
                  </div>
                </div>
                <div className="bg-white dark:bg-slate-800 rounded-xl shadow-lg p-6">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
                ...
                <div className="lg:col-span-2 bg-white dark:bg-slate-800 rounded-xl shadow-lg p-6">
                  ...
                  <ResponsiveContainer width="100%" height={350}>
                    <PieChart><Pie data={energyDistribution} cx="50%" cy="50%" labelLine={false} label={({ name, value }) => `${name}: ${(value / (realTimeData.aiWorkloadPower + realTimeData.coolingPower + realTimeData.infrastructurePower) * 100).toFixed(0)}%`} outerRadius={120} fill="#8884d8" dataKey="value">{energyDistribution.map((entry) => (<Cell key={entry.name} fill={entry.color} />))}</Pie><Tooltip contentStyle={{ backgroundColor: '#334155', border: 'none' }} /></PieChart>
                  </ResponsiveContainer>
                </div>
              </div>
            </div>
                ...
                <div className="bg-white dark:bg-slate-800 rounded-xl shadow-lg p-6">
                  ...
                  <ResponsiveContainer width="100%" height={250}>
                    <BarChart data={aiWorkloadBreakdown} layout="vertical" margin={{ left: 110 }}>
                      ...
                      <YAxis type="category" dataKey="category" width={110} tick={{ fill: 'rgb(156 163 175)', fontSize: 12 }} />
